...
class Client extends Base
{
    ...
    {
        \ElasticSearchClient::connection()->setIndex('index')->delete();
        \ElasticSearchClient::connection()->setIndex('index2')->delete();
        \ElasticSearchClient::connection()->setIndex('test-index')->delete();
        \ElasticSearchClient::connection()->delete();
    }

    public function testDsnIsCorrectlyParsed()
    {
        $search = \ElasticSearchClient::connection('http://test.com:9100/index/type');
        $config = array(
            'protocol' => 'http',
        ...
        $this->assert->array($search->configuration())
            ->isEqualTo($config);
    }
    ...
    /**
     * @expectedException \ElasticSearchHTTPException
     */
    public function testSearchThrowExceptionWhenServerDown()
    {
        $client = \ElasticSearchClient::connection(array(
            'servers' => array(
                '127.0.0.1:9201'
            )
        ));
