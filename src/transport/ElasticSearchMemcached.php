 ...
 */
class ElasticSearchMemcached extends ElasticSearchBase
{
    /**
     * @var Memcache
     */
    ...
    {
        if ($id === false)
            throw new ElasticSearchException("Memcached transport requires id when indexing");
    ...
    public function search($query, $options = array())
    {
        if (is_array($query)) {
            if (array_key_exists("query", $query)) {
                $dsl = new ElasticSearchStringify($query);
                $q = (string)$dsl;
                $url = $this->buildUrl(
                    array(
